...
** demographics
	...
	lab def ost 0 "West" 1 "Ost" , replace
	lab val ost ost
	tab1 ost sampreg

	cap drop frau
	recode sex 1 = 0 2 = 1, gen(frau)
	lab def frau 0 "männlich" 1 "weiblich" , replace
	lab val frau frau
	tab1 frau sex
...
** Job Status
	...
	lab def erwstatus 1 "Vollzeit" 2 "Teilzeit", replace
...
Because SOEP Data are not available in R format we can read them into R wth readstata13 or haven and save them as rds for future use.
...
ex_mydf <- mydf_raw %>% 
      # Demographics
      mutate(ost = sampreg - 1, # ost(1), west(0)
             # age
             alter = syear - gebjahr, 
             # gender
             frau = sex -1, # frau = 1 mann = 0
